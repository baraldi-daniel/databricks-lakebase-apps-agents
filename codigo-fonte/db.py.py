...
# ---- Smart Cache: detect specific/personal queries ----

def is_cacheable_query(query_text):
    """Detect if a query is generic (cacheable) or specific (not cacheable).
    Only cache longer, self-contained questions. Short/contextual messages skip cache."""
    text = query_text.strip()

    # Too short to be a standalone question — context-dependent
    if len(text) < 15:
        return False

    # Short conversational replies that depend on context
    short_replies = {
        "sim", "nao", "não", "ok", "quero", "pode", "certo", "claro",
        "isso", "exato", "confirmo", "confirma", "beleza", "fechar",
        "esse", "essa", "este", "esta", "aquele", "aquela",
        "por favor", "obrigado", "obrigada", "valeu", "top",
        "pode ser", "vamos", "bora", "fechado", "combinado",
        "sim por favor", "sim quero", "nao obrigado", "nao quero",
    }
    if text.lower().rstrip("!.?,") in short_replies:
        return False
    ...
    # Order/sale number pattern (#123, venda 45, pedido 78)
    if re.search(r'#\d+|venda\s+\d+|pedido\s+\d+|compra\s+\d+', text, re.IGNORECASE):
        return False

    # Specific quantity + product references ("3 iphones", "comprar 2")
    if re.search(r'comprar\s+\d+|quero\s+\d+|registr\w+\s+\w+\s+venda', text, re.IGNORECASE):
        return False

    # Name patterns: "meu nome e X", "cliente X", "para X", "do X" where X is capitalized
    if re.search(r'(meu nome|me chamo|cliente|para|do|da)\s+[A-Z][a-z]+', text):
        return False

    # If query has "meu", "minha", "meus" (personal reference)
    if re.search(r'\b(meu|minha|meus|minhas)\b', text, re.IGNORECASE):
        return False
...
def save_gmail_token(conn, user_email, access_token, refresh_token, token_expiry, ttl_hours=720):
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO gmail_tokens (user_email, access_token, refresh_token, token_expiry, ttl_hours, updated_at)
        ...
        ON CONFLICT (user_email) DO UPDATE SET
            ...
            updated_at = NOW()
    """, (user_email, access_token, refresh_token, token_expiry, ttl_hours))
    cur.close()


def get_gmail_token(conn, user_email):
    cur = conn.cursor()
    cur.execute("""
        SELECT access_token, refresh_token, token_expiry, ttl_hours, created_at
        FROM gmail_tokens WHERE user_email = %s
    """, (user_email,))
    row = cur.fetchone()
    cur.close()
    if not row:
        return None
    return {
        "access_token": row[0], "refresh_token": row[1],
        "token_expiry": row[2], "ttl_hours": row[3], "created_at": row[4],
    }


def delete_gmail_token(conn, user_email):
    cur = conn.cursor()
    cur.execute("DELETE FROM gmail_tokens WHERE user_email = %s", (user_email,))
    cur.close()
